...
- [📘 Linux サーバー構築 基本設計書](#-linux-サーバー構築-基本設計書)
  ...
  - [🔷 7. 主要サービス基本設計](#-7-主要サービス基本設計)
    - [▶️ 7.1 DNS（BIND 9.16 / named）](#️-71-dnsbind-916--named)
    - [▶️ 7.2 Webサーバー（Apache httpd 2.4.57 / Nginx 1.22）](#️-72-webサーバーapache-httpd-2457--nginx-122)
    - [▶️ 7.3 プロキシ（Squid 5.5）](#️-73-プロキシsquid-55)
    - [▶️ 7.4 ファイル共有（Samba 4.17 / NFS v4.2）](#️-74-ファイル共有samba-417--nfs-v42)
    - [▶️ 7.5 メールサービス（Postfix 3.5.9 / Dovecot 2.3.16）](#️-75-メールサービスpostfix-359--dovecot-2316)
    - [▶️ 7.6 DHCP / ディレクトリ / FTP / VPN](#️-76-dhcp--ディレクトリ--ftp--vpn)
    - [▶️ 7.7 暗号化設計方針](#️-77-暗号化設計方針)
...
### ▶️ 7.1 DNS（BIND 9.16 / named）

| 項目 | 設計方針 |
|------|---------|
| バージョン | **BIND 9.16**（RHEL9 AppStream） |
| 動作モード | 権威DNS または キャッシュDNS（用途に応じて選択） |
| 設定ファイル | `/etc/named.conf`（メイン）、`/var/named/`（ゾーンファイル） |
| DNSSEC | `dnssec-policy` ディレクティブ（BIND 9.16の新機能）を使用 |
| セキュリティ | `recursion no`（権威DNS）/ `recursion yes; allow-recursion { 内部NW; };`（キャッシュDNS） |
| **注意** | BIND9.16で `dnssec-enable` / `dnssec-lookaside` は廃止。`dnssec-validation auto;` のみ使用 |

### ▶️ 7.2 Webサーバー（Apache httpd 2.4.57 / Nginx 1.22）
...
### ▶️ 7.4 ファイル共有（Samba 4.17 / NFS v4.2）
...
### ▶️ 7.5 メールサービス（Postfix 3.5.9 / Dovecot 2.3.16）
...
| 項目 | 設計方針 |
|------|---------|
| 対象サービス | httpd / nginx / postfix / named / sshd / chronyd / auditd |
