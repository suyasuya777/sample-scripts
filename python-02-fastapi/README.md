...
| フォルダ | 内容 |
|---|---|
| [`01_basics_and_structure/`](#01_basics_and_structure) | 基礎・構造（プロジェクト構成・ルーター分割・設定管理・DI） |
| [`02_request_response/`](#02_request_response) | リクエスト・レスポンス（Pydantic・バリデーション・ページネーション） |
| [`03_auth_security/`](#03_auth_security) | 認証・セキュリティ（JWT・OAuth2・PBKDF2・RBAC） |
| [`04_database/`](#04_database) | DB連携（SQLAlchemy・ORM・CRUD・Alembic） |
...
| [`08_testing/`](#08_testing) | テスト（pytest・TestClient・フィクスチャ・DI差し替え） |
