...
async def find_by_user(user_id: int, db: Session = Depends(get_db)):
    return item_crud.get_items_by_user(db, user_id)


@router.post("", response_model=ItemResponse)
async def create(item_in: ItemCreate, db: Session = Depends(get_db)):
