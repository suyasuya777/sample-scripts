...
| フォルダ | 内容 |
|---|---|
| [`01_request_response/`](#01_request_response) | リクエスト・レスポンス（バリデーション・ページネーション・レスポンス設計） |
| [`02_auth_security/`](#02_auth_security) | 認証・セキュリティ（APIキー・RBAC） |
| [`03_async_performance/`](#03_async_performance) | 非同期・パフォーマンス（BackgroundTasks・外部API呼び出し） |
| [`04_error_handling/`](#04_error_handling) | エラーハンドリング（HTTPException・バリデーションエラー） |
| [`05_middleware/`](#05_middleware) | ミドルウェア・共通処理（ロギング・構造化ログ） |
| [`06_deploy/`](#06_deploy) | 運用・デプロイ（Docker・ヘルスチェック・CloudWatch） |
...
| カテゴリ | ディレクトリ | 日本語タイトル |
|---|---|---|
| [`01_request_response/`](#01_request_response) | [`pagination/`](#pagination) | ページネーション（offset/limit・カーソル方式） |
...
| [`02_auth_security/`](#02_auth_security) | [`api_key_authentication/`](#api_key_authentication) | APIキー認証（ヘッダー・クエリパラメーター） |
| | [`role_based_access_control/`](#role_based_access_control) | ロールベースアクセス制御（RBAC） |
| [`03_async_performance/`](#03_async_performance) | [`background_tasks/`](#background_tasks) | バックグラウンドタスク（レスポンス後の非同期処理） |
| | [`external_api_call_with_httpx/`](#external_api_call_with_httpx) | 外部API非同期呼び出し（httpx） |
| [`04_error_handling/`](#04_error_handling) | [`http_exception_usage/`](#http_exception_usage) | HTTPException の使い方とステータスコード定数 |
| | [`validation_error_response_format/`](#validation_error_response_format) | バリデーションエラーレスポンスの整形 |
| [`05_middleware/`](#05_middleware) | [`request_logging_middleware/`](#request_logging_middleware) | リクエストログ記録ミドルウェア |
| | [`structured_logging/`](#structured_logging) | 構造化ログ（JSON形式・CloudWatch対応） |
| [`06_deploy/`](#06_deploy) | [`cloudwatch_logging/`](#cloudwatch_logging) | CloudWatch Logsへのログ送信（watchtower） |
...
### <a id="01_request_response"></a>01_request_response/（リクエスト・レスポンス）　―　入力値の検証・ページング・レスポンス形式の設計パターンを学ぶ
...
<a id="pagination"></a>
#### [pagination.py](01_request_response/pagination/pagination.py)　―　ページネーション（offset/limit・カーソル方式）
...
#### [response_model_filtering.py](01_request_response/response_model_filtering/response_model_filtering.py)　―　レスポンスフィールドの絞り込みと機密情報除外
...
#### [unified_response_format.py](01_request_response/unified_response_format/unified_response_format.py)　―　統一レスポンスフォーマット（Generic[T]）
...
<a id="validation"></a>
#### [validation.py](01_request_response/validation/validation.py)　―　バリデーション（フィールド制約・カスタムバリデーター）
...
### <a id="02_auth_security"></a>02_auth_security/（認証・セキュリティ）　―　APIキー認証・JWT・RBACによるエンドポイント保護パターンを学ぶ
...
#### [api_key_authentication.py](02_auth_security/api_key_authentication/api_key_authentication.py)　―　APIキー認証（ヘッダー・クエリパラメーター）
...
#### [role_based_access_control.py](02_auth_security/role_based_access_control/role_based_access_control.py)　―　ロールベースアクセス制御（RBAC）
...
### <a id="03_async_performance"></a>03_async_performance/（非同期・パフォーマンス）　―　レスポンスをブロックしない非同期処理・外部API呼び出しパターンを学ぶ

```
03_async_performance/
├── background_tasks/
│   └── background_tasks.py
...
#### [background_tasks.py](03_async_performance/background_tasks/background_tasks.py)　―　バックグラウンドタスク（レスポンス後の非同期処理）
...
#### [external_api_call_with_httpx.py](03_async_performance/external_api_call_with_httpx/external_api_call_with_httpx.py)　―　外部API非同期呼び出し（httpx）
...
### <a id="04_error_handling"></a>04_error_handling/（エラーハンドリング）　―　HTTPException・バリデーションエラーを統一フォーマットで返すパターンを学ぶ
...
#### [http_exception_usage.py](04_error_handling/http_exception_usage/http_exception_usage.py)　―　HTTPException の使い方とステータスコード定数
...
#### [validation_error_response_format.py](04_error_handling/validation_error_response_format/validation_error_response_format.py)　―　バリデーションエラーレスポンスの整形
...
### <a id="05_middleware"></a>05_middleware/（ミドルウェア）　―　全リクエストに横断的に適用するログ記録・JSON構造化ログのパターンを学ぶ
...
#### [request_logging_middleware.py](05_middleware/request_logging_middleware/request_logging_middleware.py)　―　リクエストログ記録ミドルウェア
...
#### [structured_logging.py](05_middleware/structured_logging/structured_logging.py)　―　構造化ログ（JSON形式・CloudWatch対応）
...
### <a id="06_deploy"></a>06_deploy/（運用・デプロイ）　―　Docker・ヘルスチェック・CloudWatchなどAWS実務に直結するデプロイ設計を学ぶ
...
#### [cloudwatch_logging.py](06_deploy/cloudwatch_logging/cloudwatch_logging.py)　―　CloudWatch Logsへのログ送信（watchtower）
...
<a id="docker"></a>
#### [docker/](06_deploy/docker/)　―　Dockerコンテナ化（Dockerfile・docker-compose）
...
#### [health_check_endpoint.py](06_deploy/health_check_endpoint/health_check_endpoint.py)　―　ヘルスチェックエンドポイント（ECS/ALB対応）
...
#### [startup_shutdown_events.py](06_deploy/startup_shutdown_events/startup_shutdown_events.py)　―　起動・終了イベント管理（lifespanパターン）
