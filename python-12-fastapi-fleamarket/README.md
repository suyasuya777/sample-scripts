...
| ディレクトリ | ソース | 説明 |
|---|---|---|
| ルート | [docker-compose.yml](#docker-composeyml--開発用db環境postgresql--pgadmin) | 開発用DB環境（PostgreSQL + pgAdmin） |
...
接続URL（`config.py` の `sqlalchemy_database_url`）は非同期ドライバを使い、以下の形式になる：
