...
### 🔹 Hooks 設定ファイルの場所（スコープ別）

Hooks は `settings.json` に記述する。配置するファイルによって適用範囲（スコープ）が変わる。

| スコープ | 保存先 | 範囲 |
|---|---|---|
| ユーザー設定（userスコープ） | `~/.claude/settings.json` | 自分のみ・全プロジェクト共通 |
| プロジェクト設定（projectスコープ） | `project-root/.claude/settings.json` | チーム共有（リポジトリにコミット） |
...
MCP サーバーの定義は `settings.json` ではなく、以下の専用ファイルに記述する。配置するファイルによって適用範囲（スコープ）が変わる。

| スコープ | 保存先 | 範囲 |
|---|---|---|
| ユーザー設定（userスコープ） | `~/.claude.json` | 自分のみ・全プロジェクト共通 |
| プロジェクト設定（projectスコープ） | `project-root/.mcp.json` | チーム共有（リポジトリにコミット） |

> `~/.claude.json` は `~/.claude/` ディレクトリの中ではなくホーム直下に置かれる点に注意（混同しやすい）。
