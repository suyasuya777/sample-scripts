...
| [rest_api_server_sqlite.py](#rest_api_server_sqlitepy-サーバ側のrest-api処理sqlite) | サーバ側のREST API処理（sqlite） |
| [rest_api_server_postgresql.py](#rest_api_server_postgresqlpy-サーバ側のrest-api処理postgresql) | サーバ側のREST API処理（postgresql） |
| [rest_api_server_mysql.py](#rest_api_server_mysqlpy-サーバ側のrest-api処理mysql) | サーバ側のREST API処理（mysql） |
...
## [rest_api_server_sqlite.py](rest_api_server_sqlite.py) サーバ側のREST API処理（sqlite）
...
## [rest_api_server_postgresql.py](rest_api_server_postgresql.py) サーバ側のREST API処理（postgresql）

**インポートするモジュール**
```python
import psycopg2                # pip install psycopg2-binary
import psycopg2.extras
from flask import Flask, request, g  # pip install flask
```
...
- ステータスコードの設定（`200`, `201`, `400`, `404`）
- エラーハンドリング（条件チェック + 直接 `return`）
- DB接続管理（`psycopg2`, `g`, `@app.teardown_appcontext`）
- プレースホルダ（`%s`）によるパラメータバインド
- `with db.cursor() as curs` によるカーソル管理
- `SERIAL PRIMARY KEY` による自動採番
- サーバの起動（`app.run`）

---

## [rest_api_server_mysql.py](rest_api_server_mysql.py) サーバ側のREST API処理（mysql）
...
- ステータスコードの設定（`200`, `201`, `400`, `404`）
- エラーハンドリング（条件チェック + 直接 `return`）
- DB接続管理（`pymysql`, `g`, `@app.teardown_appcontext`）
- プレースホルダ（`%s`）によるパラメータバインド
- `with db.cursor() as curs` によるカーソル管理
- `AUTO_INCREMENT PRIMARY KEY` による自動採番
- 文字コード設定（`charset=utf8mb4`）
- サーバの起動（`app.run`）
