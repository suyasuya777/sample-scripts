...
| ディレクトリ | ソース名 | 説明 |
|---|---|---|
| source_project/ | [main.py](#mainpy--fastapi-エントリーポイントcorsルーター登録) | FastAPI エントリーポイント・CORS・ルーター登録 |
...
| `fastapi.Request` | カスタム例外ハンドラーのリクエスト受け取り |
...
| `MemoDescription` | `str \| None` | 詳細説明（任意・デフォルト: `None`） |
| `MemoPriority` | `str` | 優先度（例: `"高"`） |
| `MemoDueDate` | `datetime \| None` | 期限日（デフォルト: `None`） |
| `MemoIsCompleted` | `bool` | 完了フラグ（デフォルト: `False`） |
| `MemoResponseMessage` | `str` | API 操作結果メッセージ |
...
| スキーマ名 | 用途 |
|---|---|
| `MemoBase` | 新規登録・更新リクエストのベーススキーマ（`title`・`description`・`priority`・`due_date`・`is_completed` をフラットに定義） |
| `Memo` | メモ情報レスポンス用スキーマ（`MemoBase` に `id`・`created_at`・`updated_at` を追加） |
| `MemoResponse` | 登録・更新・削除の結果メッセージ返却用スキーマ（`message` フィールドのみ） |
...
| `schemas.memo` | リクエストスキーマ（`MemoBase`） |
...
| `get_memo_by_id(db_session, id)` | GET | `where(Memo.id == id)` で 1 件取得（存在しない場合は `None`） |
| `update_memo(db_session, id, memo_data)` | PUT | ID で取得後、各フィールドを上書きして `commit` → `refresh` |
| `delete_memo(db_session, id)` | DELETE | ID で取得後、`db_session.delete` → `commit` で削除 |
...
| メソッド | パス | パスパラメータ | 処理内容 |
|---|---|---|---|
| `POST` | `/memos/` | ─ | メモ新規登録。成功時に `MemoResponse`（登録完了メッセージ）を返却 |
| `GET` | `/memos/` | ─ | メモ全件取得。ORM オブジェクトを `Memo` スキーマのリストに変換して返却 |
| `GET` | `/memos/{id}` | `id: int` | 指定 ID のメモ 1 件取得。存在しない場合は 404 |
| `PUT` | `/memos/{id}` | `id: int` | 指定 ID のメモを更新。存在しない場合は 404 |
| `DELETE` | `/memos/{id}` | `id: int` | 指定 ID のメモを削除。存在しない場合は 404 |
...
| 関数名 | 処理内容 |
|---|---|
| `displayMessage(message)` | アラートダイアログでメッセージを表示 |
| `resetForm()` | フォームを初期状態（新規登録モード）にリセット |
| `createMemo(memo)` | POST リクエストでメモを新規登録 |
| `updateMemo(memo)` | PUT リクエストで編集中のメモを更新（`editingMemoId` を使用） |
| `deleteMemo(memoId)` | DELETE リクエストでメモを削除 |
| `fetchAndDisplayMemos()` | GET リクエストでメモ一覧を取得し、テーブルに描画 |
| `editMemo(memoId)` | 指定 ID のメモを GET して、フォームに値をセット（編集モードへ切り替え） |

フォームで扱うフィールドは `title`・`description`・`priority`（セレクトボックス）・`due_date`（日付入力）・`is_completed`（チェックボックス）の 5 項目です。メモ一覧テーブルにはこれら全フィールドに加え、編集・削除ボタンの列が表示されます。
