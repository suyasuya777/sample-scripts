...
| カテゴリ | ディレクトリ | 日本語タイトル |
|---|---|---|
| `01_basics_and_structure/` | `project_structure/` | FastAPI標準プロジェクト構成 |
| `02_request_response/` | `pagination/` | ページネーション（offset/limit・カーソル方式） |
| | `response_model_filtering/` | レスポンスフィールドの絞り込みと機密情報除外 |
| | `unified_response_format/` | 統一レスポンスフォーマット（Generic[T]） |
| | `validation/` | バリデーション（フィールド制約・カスタムバリデーター） |
| `03_auth_security/` | `api_key_authentication/` | APIキー認証（ヘッダー・クエリパラメーター） |
| | `role_based_access_control/` | ロールベースアクセス制御（RBAC） |
| `04_database/` | `sqlalchemy_models_and_relationships/` | SQLAlchemyモデルとリレーション定義 |
| `05_async_performance/` | `background_tasks/` | バックグラウンドタスク（レスポンス後の非同期処理） |
| | `external_api_call_with_httpx/` | 外部API非同期呼び出し（httpx） |
| `06_error_handling/` | `custom_exception_handler/` | カスタム例外ハンドラーと統一エラーレスポンス |
| | `http_exception_usage/` | HTTPException の使い方とステータスコード定数 |
| | `validation_error_response_format/` | バリデーションエラーレスポンスの整形 |
| `07_middleware/` | `request_logging_middleware/` | リクエストログ記録ミドルウェア |
| | `structured_logging/` | 構造化ログ（JSON形式・CloudWatch対応） |
| `08_testing/` | `integration_test_with_db_mock/` | DBモックを使った統合テスト（インメモリSQLite） |
| | `pytest_fixtures_and_conftest/` | pytestフィクスチャとconftestの設計 |
| `09_deploy/` | `cloudwatch_logging/` | CloudWatch Logsへのログ送信（watchtower） |
| | `docker/` | Dockerコンテナ化（Dockerfile・docker-compose） |
| | `health_check_endpoint/` | ヘルスチェックエンドポイント（ECS/ALB対応） |
| | `s3_file_upload/` | S3ファイルアップロードとPresigned URL生成 |
| | `startup_shutdown_events/` | 起動・終了イベント管理（lifespanパターン） |
