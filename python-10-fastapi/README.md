...
| # | ディレクトリ名 | 日本語タイトル |
|---|---|---|
| 01 | `01_basics_and_structure/project_structure/` | FastAPI標準プロジェクト構成 |
| 02 | `02_request_response/pagination/` | ページネーション（offset/limit・カーソル方式） |
| 03 | `02_request_response/response_model_filtering/` | レスポンスフィールドの絞り込みと機密情報除外 |
| 04 | `02_request_response/unified_response_format/` | 統一レスポンスフォーマット（Generic[T]） |
| 05 | `02_request_response/validation/` | バリデーション（フィールド制約・カスタムバリデーター） |
| 06 | `03_auth_security/api_key_authentication/` | APIキー認証（ヘッダー・クエリパラメーター） |
| 07 | `03_auth_security/role_based_access_control/` | ロールベースアクセス制御（RBAC） |
| 08 | `04_database/sqlalchemy_models_and_relationships/` | SQLAlchemyモデルとリレーション定義 |
| 09 | `05_async_performance/background_tasks/` | バックグラウンドタスク（レスポンス後の非同期処理） |
| 10 | `05_async_performance/external_api_call_with_httpx/` | 外部API非同期呼び出し（httpx） |
| 11 | `06_error_handling/custom_exception_handler/` | カスタム例外ハンドラーと統一エラーレスポンス |
| 12 | `06_error_handling/http_exception_usage/` | HTTPException の使い方とステータスコード定数 |
| 13 | `06_error_handling/validation_error_response_format/` | バリデーションエラーレスポンスの整形 |
| 14 | `07_middleware/request_logging_middleware/` | リクエストログ記録ミドルウェア |
| 15 | `07_middleware/structured_logging/` | 構造化ログ（JSON形式・CloudWatch対応） |
| 16 | `08_testing/integration_test_with_db_mock/` | DBモックを使った統合テスト（インメモリSQLite） |
| 17 | `08_testing/pytest_fixtures_and_conftest/` | pytestフィクスチャとconftestの設計 |
| 18 | `09_deploy/cloudwatch_logging/` | CloudWatch Logsへのログ送信（watchtower） |
| 19 | `09_deploy/docker/` | Dockerコンテナ化（Dockerfile・docker-compose） |
| 20 | `09_deploy/health_check_endpoint/` | ヘルスチェックエンドポイント（ECS/ALB対応） |
| 21 | `09_deploy/s3_file_upload/` | S3ファイルアップロードとPresigned URL生成 |
| 22 | `09_deploy/startup_shutdown_events/` | 起動・終了イベント管理（lifespanパターン） |
...
| [`04_database/`](#04_database) | DB連携（SQLAlchemy・リレーション） |
| [`05_async_performance/`](#05_async_performance) | 非同期・パフォーマンス（BackgroundTasks・外部API呼び出し） |
| [`06_error_handling/`](#06_error_handling) | エラーハンドリング（カスタム例外・HTTPException） |
| [`07_middleware/`](#07_middleware) | ミドルウェア・共通処理（ロギング・構造化ログ） |
| [`08_testing/`](#08_testing) | テスト（pytest・フィクスチャ・DBモック） |
...
### <a id="04_database"></a>04_database/（DB連携）　―　SQLAlchemyによるモデル定義・リレーション・タイムスタンプ自動設定を学ぶ
...
#### [sqlalchemy_models_and_relationships.py](04_database/sqlalchemy_models_and_relationships/sqlalchemy_models_and_relationships.py)　―　SQLAlchemyモデルとリレーション定義
...
`Enum` 型カラム・`created_at`/`updated_at` 自動設定・`ondelete=CASCADE` の詳細解説。project_structure のシンプルなモデル定義に対する発展内容。

---

### <a id="05_async_performance"></a>05_async_performance/（非同期・パフォーマンス）　―　レスポンスをブロックしない非同期処理・外部API呼び出しパターンを学ぶ
...
#### [background_tasks.py](05_async_performance/background_tasks/background_tasks.py)　―　バックグラウンドタスク（レスポンス後の非同期処理）
...
#### [external_api_call_with_httpx.py](05_async_performance/external_api_call_with_httpx/external_api_call_with_httpx.py)　―　外部API非同期呼び出し（httpx）
...
### <a id="06_error_handling"></a>06_error_handling/（エラーハンドリング）　―　カスタム例外・HTTPException・バリデーションエラーを統一フォーマットで返すパターンを学ぶ
...
#### [custom_exception_handler.py](06_error_handling/custom_exception_handler/custom_exception_handler.py)　―　カスタム例外ハンドラーと統一エラーレスポンス
...
#### [http_exception_usage.py](06_error_handling/http_exception_usage/http_exception_usage.py)　―　HTTPException の使い方とステータスコード定数
...
#### [validation_error_response_format.py](06_error_handling/validation_error_response_format/validation_error_response_format.py)　―　バリデーションエラーレスポンスの整形
...
### <a id="07_middleware"></a>07_middleware/（ミドルウェア）　―　全リクエストに横断的に適用するログ記録・JSON構造化ログのパターンを学ぶ
...
#### [request_logging_middleware.py](07_middleware/request_logging_middleware/request_logging_middleware.py)　―　リクエストログ記録ミドルウェア
...
#### [structured_logging.py](07_middleware/structured_logging/structured_logging.py)　―　構造化ログ（JSON形式・CloudWatch対応）
...
### <a id="08_testing"></a>08_testing/（テスト）　―　インメモリDBを使った統合テスト・pytestフィクスチャの設計パターンを学ぶ
...
#### [integration_test_with_db_mock.py](08_testing/integration_test_with_db_mock/integration_test_with_db_mock.py)　―　DBモックを使った統合テスト（インメモリSQLite）
...
#### [pytest_fixtures_and_conftest.py](08_testing/pytest_fixtures_and_conftest/pytest_fixtures_and_conftest.py)　―　pytestフィクスチャとconftestの設計
...
### <a id="09_deploy"></a>09_deploy/（運用・デプロイ）　―　Docker・ヘルスチェック・S3・CloudWatchなどAWS実務に直結するデプロイ設計を学ぶ
...
#### [cloudwatch_logging.py](09_deploy/cloudwatch_logging/cloudwatch_logging.py)　―　CloudWatch Logsへのログ送信（watchtower）
...
#### [docker/](09_deploy/docker/)　―　Dockerコンテナ化（Dockerfile・docker-compose）
...
#### [health_check_endpoint.py](09_deploy/health_check_endpoint/health_check_endpoint.py)　―　ヘルスチェックエンドポイント（ECS/ALB対応）
...
#### [s3_file_upload.py](09_deploy/s3_file_upload/s3_file_upload.py)　―　S3ファイルアップロードとPresigned URL生成
...
#### [startup_shutdown_events.py](09_deploy/startup_shutdown_events/startup_shutdown_events.py)　―　起動・終了イベント管理（lifespanパターン）
